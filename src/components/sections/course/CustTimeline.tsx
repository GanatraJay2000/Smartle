        ...
        list.length > 1 ? (<>
          {
            list.map((item: string, itemIdx: number) => {
              let myDir = ["up", ""];
              if (itemIdx % 2 == 0) myDir = ["down", "-"];
              let showLine = "transition-all hidden";
              if (itemIdx === activeDot) showLine = "transition-all ";                
              return (<React.Fragment key={itemIdx}>
                <div onClick={() => setActiveDot(itemIdx)} className={`h-5 w-5 bg-color-400 rounded-full ${myDir[1]}translate-y-10 cursor-pointer relative`}>
                  <div className={`absolute ${showLine} w-20 -left-5 -top-10 font-semibold`}>
                    Week {itemIdx + 1}
                  </div>
